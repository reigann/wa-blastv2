...
export default function Templates() {
  ...
  const sorted = useMemo(() => {
    return templates.slice().sort((a, b) => new Date(b.updated_at || b.created_at) - new Date(a.updated_at || a.created_at));
  }, [templates]);

  const existingMediaVisible = editing?.media_path && !removeExistingMedia && !attachmentFile;
      ...
      <PageHeader
        ...
        actions={[
          <Button key="new" onClick={openCreate}>
            <i className="bi bi-plus-lg me-2" />New Template
          </Button>,
        ]}
      />
              ...
              <Card.Body>
                <div className="d-flex justify-content-between align-items-start mb-3">
                  ...
                  <Dropdown align="end">
                    ...
                    <Dropdown.Menu>
                      <Dropdown.Item onClick={() => openEdit(template)}>Edit</Dropdown.Item>
                      <Dropdown.Item onClick={() => duplicateTemplate(template)}>Duplicate</Dropdown.Item>
                      <Dropdown.Item className="text-danger" onClick={() => deleteTemplate(template.id)}>
                        Delete
                      </Dropdown.Item>
                    </Dropdown.Menu>
                  </Dropdown>
                ...
                <h3 className="mb-2" style={{ fontSize: 16 }}>{template.name}</h3>
                <p
                  ...
                  <div className="mb-3 p-2 rounded border bg-light-subtle">
                    {isImageType(template.media_type || '') ? (
                      ...
                      <div className="d-flex align-items-center gap-2 small">
                        ...
                        <span className="text-truncate">{template.media_name || 'Document attached'}</span>
                      </div>
                    )}
                  </div>
                ) : null}
                ...
                <div className="d-flex flex-wrap gap-1 mb-3">
                  {variableOptions
                    .filter((token) => template.content?.toLowerCase().includes(token.replace(/[{}]/g, '').toLowerCase()))
                    .map((token) => (
                      <span key={`${template.id}-${token}`} className="badge" style={{ background: '#ecfdf3', color: '#166534' }}>
                        {token}
                      </span>
                    ))}
                </div>
                ...
                <div className="d-flex justify-content-between small text-secondary">
                  <span>{new Date(template.updated_at || template.created_at || Date.now()).toLocaleDateString()}</span>
                  <span>{template.media_path ? 'With attachment' : `${(template.id % 9) + 1} uses`}</span>
                </div>
              </Card.Body>
            </Card>
          ))}
          ...
          <Row className="g-3">
            <Col md={7}>
                ...
                <div className="d-flex flex-wrap gap-2 mt-2">
                  {variableOptions.map((token) => (
                    <Button
                      ...
                      onClick={() => setForm((prev) => ({ ...prev, content: insertAtCursor(prev.content, token) }))}
                    >
                      {token}
                    </Button>
                  ))}
                </div>
                <small className="text-secondary d-block mt-2">{form.content.length} characters</small>
              </Form.Group>
              ...
              <Form.Group className="mb-0">
                ...
                />
                <small className="text-secondary d-block mt-1">Allowed: image, PDF, DOC, XLS, PPT, TXT (max 16MB)</small>
              </Form.Group>
            </Col>

            <Col md={5}>
              <div className="p-3 rounded-3 border mb-3" style={{ background: '#ece5dd' }}>
                <div className="rounded-3 bg-white p-3 shadow-sm" style={{ maxWidth: 280 }}>
                  <div className="small fw-semibold mb-1">Preview</div>
                  <div style={{ whiteSpace: 'pre-wrap' }}>{form.content || 'Your message preview appears here...'}</div>
                </div>
              </div>
              ...
              {attachmentFile ? (
                <div className="p-2 border rounded-3">
                  {isImageType(attachmentFile.type) ? (
                    ...
                    <div className="d-flex align-items-center gap-2 small">
                      <i className="bi bi-file-earmark-text" />
                      <span className="text-truncate">{attachmentFile.name}</span>
                    </div>
                  )}
                  <Button variant="light" className="btn-outline-soft btn-sm mt-2" onClick={clearAttachmentSelection}>
                    Remove Attachment
                  </Button>
                </div>
              ) : null}
              ...
              {existingMediaVisible ? (
                <div className="p-2 border rounded-3">
                  {isImageType(editing.media_type || '') ? (
                    ...
                    <div className="d-flex align-items-center gap-2 small">
                      <i className="bi bi-file-earmark-text" />
                      <span className="text-truncate">{editing.media_name || 'Document attached'}</span>
                    </div>
                  )}
                  <Button variant="light" className="btn-outline-soft btn-sm mt-2" onClick={clearAttachmentSelection}>
